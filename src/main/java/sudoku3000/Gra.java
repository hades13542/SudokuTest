...
public class Gra extends Generator implements Serializable {
	...
	private static final byte[] keyValue = new byte[] { 'g', '5', 'r', '@', 'g', '8', '[', '/', '7', 'e', '<', '*', '4',
			'l', ',', '!' }; // 128-bit klucz
	...
	/**
	 * funckja serializuje, szyfruje i zapisuje do pliku binarnego obiekt, na
	 * rzecz którego jest wywoływana jako argument przyjmuje string, który
	 * jest ścieżką zapisu pierwsze cztery bajty zapisanego pliku nie są
	 * szyfrowane i stanowią informacje o ilości bajtów w pozostałej
	 * części pliku
	 * 
	 * @param nazwa
	 *            nazwa pliku do którego zostanie zapisana gra
	...
	/**
	 * funkcja wczytuje grę z zaszyfrowanego pliku binarnego jako argument
	 * przyjmuje ścięzke do wczytywanego pliku
	 * 
	 * @param nazwa
	...
	public static Gra wczytaj(String nazwa) throws Exception {
		...
		try {
			...
			int rozmiar = strumien.readInt(); // odczytujemy pierwsze 4 najty
												// pliku, które zawierają
												// informację o ropzmiarze
												// pliku
			...
			for (int i = nazwa.length() - 1; i >= 0; i--) {
				if (nazwa.charAt(i) == File.separator.charAt(0)) {
					nazwa = nazwa.substring(i + 1, nazwa.length()); // do pola
																	// nazwa
																	// wprowadzamy
																	// nazwe
					break;
	...
	/**
	 * @return true zwracva true jeśli plansza, na której toczy się gra jest
	 *         identyczna z planszą z rozwiązanie sudoku
	 */
	 ...
	 * @param col
	 *            - kolumna
	 * @return true zwraca true jeśli liczbę pod daną pozycją można
	 *         zmienić
	 */
	public boolean czyMozna(int row, int col) // sprawdza czy liczbe pod danymi
												// wspolrzednymi mozna zmienic
	{
	...
	/**
	 * funkcja losuje podpowiedź w przypadku braku dostępnych podpowiedzi
	 * zwraca o tym informacje
	 * 
	 * @return String w ustalonym formacie z informacją, które pole zostąło
	 *         podpowiedzniane
	 */
